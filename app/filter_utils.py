...
def apply_arb_filters(arbs: List[dict], filters: dict, tier: str) -> List[dict]:
    ...
    logger.debug(
        f"max_profit filter value: {max_profit} (type: {type(max_profit).__name__})"
    )
    if max_profit is not None:
        try:
            ...
            logger.debug(
                f"max_profit filter applied: {before_count} -> {len(filtered)} arbs (max: {max_profit_val}%)"
            )
    ...
    if league:
        if isinstance(league, str):
            ...
            filtered = [
                arb for arb in filtered if arb.get("league", "").upper() in league_set
            ]
    ...
    if market_type:
        ...
        if market_types:
            filtered = [
                arb for arb in filtered if arb.get("market", "").lower() in market_types
            ]

    sportsbooks_filter = filters.get("sportsbooks")
    if (
        sportsbooks_filter
        and isinstance(sportsbooks_filter, list)
        and len(sportsbooks_filter) > 0
    ):
...
def apply_terminal_filters(games: List[dict], filters: dict, tier: str) -> List[dict]:
    ...
    if league:
        if isinstance(league, str):
            if league.lower() != "all":
                filtered = [
                    g for g in filtered if g.get("league", "").upper() == league.upper()
                ]
        elif isinstance(league, list) and len(league) > 0:
            league_set = set(specific_league.upper() for specific_league in league)
            filtered = [
                g for g in filtered if g.get("league", "").upper() in league_set
            ]
...
def apply_ev_filters(ev_bets: List[dict], filters: dict, tier: str) -> List[dict]:
    ...
    if allowed_leagues:
        allowed_set = set(league.upper() for league in allowed_leagues)
        filtered = [
            ev for ev in filtered if ev.get("league", "").upper() in allowed_set
        ]
    ...
    if min_ev is not None:
        try:
            ...
            if min_ev_val > 0:
                filtered = [
                    ev for ev in filtered if ev.get("expected_value", 0) >= min_ev_val
                ]
        except (ValueError, TypeError):
            pass
    ...
    if conf_filters and isinstance(conf_filters, list) and len(conf_filters) > 0:
        ...
        filtered = [
            ev for ev in filtered if ev.get("confidence", "").upper() in confidence_set
        ]
    ...
    if league:
        if isinstance(league, str):
            ...
            league_set = set(specific_league.upper() for specific_league in league)
            filtered = [
                ev for ev in filtered if ev.get("league", "").upper() in league_set
            ]
...
def apply_sportsbook_filter_to_games(
    games: List[dict], sportsbooks: Set[str]
) -> List[dict]:
    """
        ...
        for market in markets:
            ...
            for outcome in outcomes:
                ...
                if filtered_history:  # Only include outcome if it has data
                    ...
                    filtered_outcome["current_best_sportsbook"] = latest.get(
                        "sportsbook"
                    )
...
def filter_terminal_data(
    games: List[Dict], game_time: Optional[str] = None
) -> List[Dict]:
    """
